...
# 5 charts: A Sector Rotation | B Series A Crunch |
#           C Deal Size Collapse | D Cohort Outcomes |
#           E Startup Explosion (Cliffhanger)
# ============================================================
...
df_raw$market <- trimws(df_raw$X.market.)

df_raw$funding_clean <- as.numeric(
  gsub('[",\\s]', '', df_raw$X.funding_total_usd., perl = TRUE))
...
cohort_a <- df_raw[df_raw$market %in% sectors_a &
                   !is.na(df_raw$year_founded) &
                   df_raw$year_founded >= 2005 &
                   df_raw$year_founded <= 2013, ]

ct_a <- as.data.frame(table(cohort_a$year_founded, cohort_a$market),
...
ct_a$lw <- ifelse(ct_a$market %in% c("Mobile","Clean Technology"), 1.5, 0.8)
...
pA <- ggplot(ct_a, aes(x = year_founded, y = index,
  ...
  annotate("text", x = 2008.15, y = 530, label = "Financial\nCrisis",
...
seed_n <- sapply(years_b, function(yr) {
  ...
  sum(as.numeric(sub$seed) > 0, na.rm = TRUE)
})

a_n <- sapply(years_b, function(yr) {
  sub <- df_raw[!is.na(df_raw$year_founded) & df_raw$year_founded == yr, ]
  sum(as.numeric(sub$round_A) > 0, na.rm = TRUE)
})
...
pB <- ggplot() +
  ...
  geom_text(data = line_df_b,
            aes(x = factor(year), y = conversion * 8 + 60,
  ...
  scale_y_continuous(
    ...
    labels   = comma,
    sec.axis = sec_axis(~ . / 8, name = "Seed → Series A Conversion (%)",
...
# ============================================================
# CHART C — SLOPE/LINE: Deal Size Collapse (3 snapshots)
# ============================================================
...
for (s in top_s_c) {
  for (yr in yrs_c) {
    sub <- df_raw[!is.na(df_raw$year_founded) &
                  df_raw$year_founded == yr &
                  df_raw$market == s &
                  !is.na(df_raw$funding_clean) &
                  df_raw$funding_clean >= 1e4 &
                  df_raw$funding_clean <= 5e8, "funding_clean"]
    if (length(sub) > 5) {
...
pD <- ggplot(outcome_df, aes(x = factor(year), y = pct, fill = status)) +
  ...
  geom_text(
    data = outcome_df[outcome_df$status %in% c("Acquired","Closed"), ],
    aes(label = paste0(round(pct, 1), "%")),
    position = position_stack(vjust = 0.5),
    size = 3.5, fontface = "bold",
    color = c(rep(drexel_blue, 6), rep("white", 6))
  ) +
  scale_fill_manual(
...
cohort_e <- df_raw[df_raw$market %in% sectors_e &
                   !is.na(df_raw$year_founded) &
...
pE <- ggplot(count_e, aes(x = year_founded, y = count, fill = market)) +
  ...
  annotate("segment",
           x = 2013.6, xend = 2014.8, y = 900, yend = 900,
           arrow = arrow(length = unit(0.25,"cm"), type = "open"),
           linetype = "dashed", color = drexel_gray, linewidth = 0.9) +
  annotate("text", x = 14.95, y = 900,
           label = "Data ends\nhere — 2013",
           hjust = 0, size = 3, color = drexel_gray, fontface = "italic") +
  scale_fill_manual(values = colors_e, name = "Sector") +
  scale_x_continuous(breaks = 2004:2013,
                     limits = c(2003.4, 2016.5)) +
